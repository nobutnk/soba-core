...
public class ClassHierarchy implements IDynamicBindingResolver {
    ...
    private static final String JAVA_LANG_OBJECT = "java" + ClassInfo.PACKAGE_SEPARATOR + "lang"
            + ClassInfo.PACKAGE_SEPARATOR + "Object";
     ...
     * @return an array of <code>MethodInfo</code> objects representing methods that
     *         might be executed by the invocation. The return value is an empty
     *         array if no method declaration matched to the invocation.
     */
    @Override
    ...
    /**
     * Resolve dynamic binding of a virtual method call. Please note that this
     * method does not care about the detail of the method definition. If a static
     * method or an undefined private method is specified, this method may return
     * incorrect result.
     * 
     * @param className  specifies a class representing a receiver type.
     * @param methodName specifies a method name.
     * @param methodDesc specifies a method descriptor (without generics
     *                   information).
     * @return MethodInfo object representing the specified method. The return value
     *         may be an emtpy if the method is not found.
     */
    private MethodInfo[] resolveDynamicCall(String className, String methodName, String methodDesc) {
        ...
        if (topDecl == null)
            return new MethodInfo[0]; // Not found

        List<MethodInfo> result = new ArrayList<>(16);
        if (topDecl.hasMethodBody())
            result.add(topDecl);

        // We explicitly avoid array types, because arrays are not included in
        // ClassHierarchy.
                    ...
                    if ((m == null) || m.isOverridable()) {
                        ...
                        for (String c : getSubtypes(currentClass)) {
                            if (m != null && m.isPackagePrivate()) {
                                // A package-private method can be overridden by only classes in the same
                                // package.
     ...
     *                   information).
     * @return MethodInfo object representing the specified method. The return value
     *         may be null if the method is not found.
     */
     ...
     * @param signatureId identifies a method by its name and parameters. To obtain
     *                    a method signature, use MethodUtil.getMethodSignature.
     * @return MethodDecl object of the nearest ancestor (including the class
     *         specified by typeName) If no ancestor classes declare the method,
     *         MethodDecl comes from interfaces who declares the method.
     * @throws ClassNotFoundException is thrown if typeName or its ancestors are not
     *                                found in class hierarchy.
     * @throws NoSuchMethodException  is thrown if method is not found in ancestor
     ...
     * 
     * @param className specifies a class name in a field access instruction. The
     *                  class may inherit a field from its parent.
     * @param fieldName
     * @param fieldDesc
     * @return a class name that defines the field specified by the arguments. The
     *         method may return null if an owner is not found.
     */
     ...
     * @param fieldDesc
     * @return a class name.
     * @see JVM Specification Section 5.4.3.2. If two public fields are accessible
     *      from the specified className and fieldName, javac reports the ambiguous
     *      field reference as an error.
     */
    public String resolveStaticFieldOwner(String className, String fieldName, String fieldDesc) {
        ...
        if (c.getSuperClass() != null)
            return resolveStaticFieldOwner(c.getSuperClass(), fieldName, fieldDesc);
        else
            return null;
    }
    ...
    /**
     * @return a set of class names which are requested by client methods, but not
     *         involved in this class hierarchy.
     */
     ...
     * 
     * @param typeName1 specifies a type to be compared. The type name must be
     *                  registered to the class hierarchy.
    ...
    public boolean isSamePackage(String typeName1, String typeName2) {
        ...
        if (c1 == null)
            requestedClasses.add(typeName1);
        if (c2 == null)
            requestedClasses.add(typeName2);
     ...
     * @return a super class name for the specified class. This method returns null
     *         for "java/lang/Object". "java.lang.Object" is returned for an
     *         interface and an array type.
     */
     ...
     * @return interfaces implemented by the specified class. If the class has no
     *         interfaces, this method returns an empty collection. If the class is
     *         an interface and it extends another interface B, B is regarded as a
     *         super-interface of the class. If the specified class has no
     *         interfaces, the result is an empty colleciton.
     */
    ...
    /**
     * This method registers a class info object to the hierarchy. This method calls
     * registerSuperClass, registerSubtype and registerInterfaces.
     * 
    ...
    /**
     * This method allows developers to manually modify the type hierarchy. A
     * subtype of a class is a subclass. A subtype of an interface is an
