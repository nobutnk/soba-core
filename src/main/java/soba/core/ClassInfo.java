...
public class ClassInfo {
     ...
     * @param loaderLabel  specifies a label indicating a location/category for a
     *                     class.
     * @throws IOException
     */
     ...
     * 
     * @param fileName
     * @param binaryStream specifies a stream of Java bytecode. Please note that the
     *                     stream is not closed by the method; the callers must
     *                     close the stream by themselves.
     */
    public ClassInfo(String fileName, InputStream binaryStream) throws IOException {
        ...
        try {
            cr1 = new ClassReader(bytes) {
                /**
                 * This extension reduces the number of allocated strings. When reading SOBA and
                 * GNU Trove class files, 52MB of 72MB strings can be discarded.
                 */
                @Override
            ...
            throw new ClassReadFailureException(
                    "ASM ClassReader cannot parse the bytecode. " + fileName + " " + e.getLocalizedMessage());
        }
    ...
    /**
     * @return a package name. A package name is separated by "/".
     */
    ...
    /**
     * @return a class name with its package name. The class name is an internal
     *         representation; a package name is separated by "/".
     */
    ...
    /**
     * @return the file name whose containing the class data. If the class file is
     *         contained in a ZIP/JAR file, the resultant path incidates the zip
     *         file and an internal file path in the archive.
     */
    ...
    /**
     * @return a label attached to this class. This is expected to distinguish a
     *         library class or an application class.
     */
    ...
    /**
     * @return a Java source file name. The file name is a relative path. The method
     *         may return null.
     */
     ...
     * @return a <code>MethodInfo</code> object if the class declares the specified
     *         method.
     */
    public MethodInfo findMethod(String methodName, String methodDesc) {
        for (MethodInfo m : methods) {
            if (m.getMethodName().equals(methodName) && m.getDescriptor().equals(methodDesc)) {
                return m;
            }
        }
        return null;
     ...
     * @return a <code>FieldInfo</code> object if the class declares the specified
     *         field.
     */
    public FieldInfo findField(String fieldName, String fieldDesc) {
        for (FieldInfo f : fields) {
            if (f.getFieldName().equals(fieldName) && f.getDescriptor().equals(fieldDesc)) {
                return f;
            }
        }
        return null;
