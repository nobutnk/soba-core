...
public class OpcodeString {
    ...
    /**
     * This method returns a local variable index accessed by the specified
     * instruction node.
     * 
     * The same index may be used for two or more local variables. To distinguish
     * such variables, use findLocalVariable method.
     */
    ...
    /**
     * This method returns true if a specified instruction reads or writes a local
     * variable represented by the specified local variable node.
     * 
     * @param insn specifies an instruction.
     * @param var  specifies a local variable.
     * @return true if the instruction accesses the variable. If var is null, this
        ...
        if (var.index == varIndex) {
            ...
            if (isStoreOperation(insn) || isIncrementOperation(insn)) {
                if (var.start == insn.getNext())
                    ...
                    if (var.start != var.end && var.end.getPrevious() != null
    ...
    /**
     * This method returns a local variable node accessed by the specified
     * VarInsnNode (if exists).
     * 
     * This method is necessary because two or more variables may have the same
     * varIndex. It should be noted that a variable "x" becomes in-scope only AFTER
     * its variable declaration such as "C x = v;". The STORE instruction accessing
     * "x" is out of scope of the variable.
     */
    ...
    /**
     * Return true if a node is an instruction to load a return address that is
     * stored into an anonymous local variable.
     * 
     * @param node
     * @return
     */
    ...
    public static boolean isPrimitiveOperation(AbstractInsnNode node) {
        ...
        return (opcode == Opcodes.ISTORE || opcode == Opcodes.FSTORE || opcode == Opcodes.LSTORE
                || opcode == Opcodes.DSTORE || opcode == Opcodes.ILOAD || opcode == Opcodes.FLOAD
                || opcode == Opcodes.LLOAD || opcode == Opcodes.DLOAD || opcode == Opcodes.IINC);
    }

    public static boolean isStoreOperation(AbstractInsnNode node) {
        int opcode = node.getOpcode();
        return (opcode == Opcodes.ISTORE || opcode == Opcodes.ASTORE || opcode == Opcodes.FSTORE
    ...
    public static boolean isLoadOperation(AbstractInsnNode node) {
        int opcode = node.getOpcode();
        return (opcode == Opcodes.ILOAD || opcode == Opcodes.ALOAD || opcode == Opcodes.FLOAD || opcode == Opcodes.LLOAD
    ...
    /**
     * @return a string representation for a label located in a position specified
     *         by index. While label.toString() returns a different string for each
     *         execution, this method returns the same string if a label is located
     *         in the same position in a method.
     */
     ...
     * @param index  specifies the position of an instruction in the list of
     *               instructions.
    ...
    private static String[] opcodeNames = new String[] { "NOP", "ACONST_NULL", "ICONST_M1", "ICONST_0", "ICONST_1",
            ...
            "DUP2", "DUP2_X1", "DUP_X2", "SWAP", "IADD", "LADD", "FADD", "DADD", "ISUB", "LSUB", "FSUB", "DSUB", "IMUL",
            ...
            "LXOR", "IINC", "I2L", "I2F", "I2D", "L2I", "L2F", "L2D", "F2I", "F2L", "F2D", "D2I", "D2L", "D2F", "I2B",
