...
/**
 * This class implements an algorithm described in Keith D. Cooper, Timothy J.
 * Harvey and Ken Kennedy: A Simple, Fast Dominance Algorithm.
 */
public class DominanceTree {
    ...
    /**
     * Creates a new <code>DominanceTree</code> instance from a
     * <code>SingleRootDirectedGraph</code> object.
     * 
    ...
    /**
     * Visit nodes using depth-first search. Reverse-post-order traversal.
     */
    ...
    private void computeDominators() {
        ...
        // sort vertices according to the descending order of reverse post order (a
        // regular post order)
        ...
        for (int from = 0; from < base.getVertexCount(); ++from) {
            for (int to : base.getEdges(from)) {
                if (reversePostOrder[from] < reversePostOrder[to])
                    immediateDominator[to] = from;
            }
        }
