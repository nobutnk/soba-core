...
public class DirectedAcyclicGraph implements IDirectedGraph {
    ...
    /**
     * Creates a new <code>DirectedAcyclicGraph</code> instance from a specified
     * base graph. If the base graph has strongly connected components, they are
     * removed by Tarjan's algorithm.
     * 
    ...
    /**
     * @return the number of vertices. The value is the same as the base graph.
     */
    @Override
     ...
     * @return true if the ID specifies a vertex included in the DAG. If the vertex
     *         is a part of strongly connected components and excluded from the DAG,
    ...
    /**
     * @param vertexId specifies a vertex.
     * @return a vertex ID which is a representative node of the strongly connected
     *         components including the specified vertexId.
     */
    ...
    /**
     * @return a new graph with reversed edges. An edge from vertex A to vertex B in
     *         the original graph is translated into an edge from B to A in the new
     *         graph.
     */
    ...
    private void tarjanDFS(int vId) {
        ...
        data.currentIndex++;
        data.stack.push(vId);
        // System.out.println("currentIndex = " + data.currentIndex + ", stack = " +
        // data.stack.size());
