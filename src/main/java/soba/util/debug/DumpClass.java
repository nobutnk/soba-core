                ...
                if (dumpTryBlock) {
                    if (m.getMethodNode().tryCatchBlocks != null) {
                        ...
                        for (TryCatchBlockNode node : m.getMethodNode().tryCatchBlocks) {
                            System.out.println("    start=" + OpcodeString.getLabelString(m.getMethodNode(), node.start)
                                    + ", end=" + OpcodeString.getLabelString(m.getMethodNode(), node.end) + ", handler="
                                    + OpcodeString.getLabelString(m.getMethodNode(), node.handler) + " (" + node.type
                                    + ")");
                        }
